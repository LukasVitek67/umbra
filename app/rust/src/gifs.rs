...
//! recipient's IP address, and the time, to the provider.
//!
//! The provider is GIPHY, through its Tenor-compatible endpoints: Google
//! discontinued the Tenor API in January 2026 and stopped issuing keys for it.
//! Those endpoints keep Tenor's request and response shape, so what changed
//! here was the host, not the logic.
//!
...
//!
//! See `docs/GIFS.md` for what this still leaks, which is not nothing, and for
//! where this design and GIPHY's API terms disagree.

use crate::updater::http_get_via_tor;

/// The key this build ships with, baked in at compile time from
/// `NULLCHAT_GIPHY_KEY` (tools/release.ps1 reads it from a file that is not in
...
/// in it gets scraped and used by strangers until GIPHY disables it — taking
...
const BUILT_IN_KEY: Option<&str> = option_env!("NULLCHAT_GIPHY_KEY");
...
const NO_KEY: &str = "Hledání GIFů teď nefunguje: klíč ke GIPHY, se kterým se \
tahle verze dodává, služba odmítla. Můžeš použít vlastní — je zdarma: \
developers.giphy.com → Create an API Key → vložit v Nastavení → Hledání GIFů.";

/// Is this shaped like an API key at all? Catches a pasted URL or a
/// half-copied string before it becomes a request and a confusing 401.
...
pub struct GifResult {
    /// The provider's id — only ever used to fetch, never sent to a peer.
    pub id: String,
    ...
    pub width: u32,
    pub height: u32,
    /// Alt text, when the provider has one.
    pub description: String,
}

/// Search GIPHY over Tor.
///
...
pub async fn search(
    ...
    // `contentfilter=off` is the "uncensored" part of the request: on GIPHY it
    // maps to G, PG, PG-13 and R, which is everything the API will serve.
    // Nothing here filters further.
    let url = format!(
        "https://api.giphy.com/v2/search?q={q}&key={key}\
...
/// "What is popular right now", for an empty search box.
pub async fn featured(
    ...
    let key = urlencode(key);
    let url = format!(
        "https://api.giphy.com/v2/featured?key={key}\
         &limit={limit}&contentfilter=off&media_filter=gif,tinygif&client_key=nullchat"
    );
    let body = http_get_via_tor(socks_port, &url, isolation).await?;
    parse_results(&body)
...
/// encrypted file channel, and their device never learns GIPHY exists.
pub async fn fetch(socks_port: u16, url: &str, isolation: &str) -> Result<Vec<u8>, String> {
    if !is_media_url(url) {
        // Only ever fetch from where GIPHY serves media. Without this, a
...
/// Does this URL point at GIPHY's media hosts, and nowhere else?
///
/// The host is compared exactly, after being cut out of the URL — a
/// `starts_with` on the prefix would accept `https://media.giphy.com.evil.example/`,
/// and a `contains` would accept anything with the string in a query parameter.
/// GIPHY serves media from `media.giphy.com`, from numbered variants of it, and
/// from `i.giphy.com`.
fn is_media_url(url: &str) -> bool {
    let Some(rest) = url.strip_prefix("https://") else {
        return false; // plain http would also strip the transport security
    };
    ...
    match host.strip_prefix("media").and_then(|h| h.strip_suffix(".giphy.com")) {
...
/// Pull the fields we need out of the Tenor-shaped JSON the compatibility
/// endpoints return.
...
mod tests {
    ...
    /// The fetch must refuse anywhere that is not a GIPHY media host, so a
    ...
    async fn fetching_from_anywhere_else_is_refused() {
        for url in [
            ...
            "http://media.giphy.com/x.gif",           // no TLS
            "https://media.giphy.com.evil.example/x.gif", // suffix, not the host
            "https://evil.example/?u=media.giphy.com/x.gif", // host only in a query
            "https://mediax.giphy.com/x.gif",         // numbered variants only
            "https://giphy.com/x.gif",                // the site, not the media host
        ] {
    ...
    #[test]
    fn giphy_media_hosts_are_accepted() {
        for url in [
            "https://media.giphy.com/media/abc/giphy.gif",
            "https://media0.giphy.com/media/abc/giphy.gif",
            "https://media4.giphy.com/media/abc/200w.gif",
            "https://i.giphy.com/abc.gif",
        ] {
            assert!(is_media_url(url), "{url} should have been accepted");
        }
    }

    #[test]
    fn results_survive_missing_fields() {
        // The provider sometimes omits formats; those entries are skipped
        // rather than crashing the picker.
        let body = br#"{"results":[
            {"id":"1","media_formats":{"gif":{"url":"https://media.giphy.com/a.gif","dims":[200,100]},
                                       "tinygif":{"url":"https://media.giphy.com/a-s.gif"}},
        ...
        assert_eq!(out.len(), 1);
        assert_eq!(out[0].gif_url, "https://media.giphy.com/a.gif");
    ...
    async fn a_missing_key_is_explained_not_requested() {
        ...
        for key in ["", "   ", "paste-your-key-here", "https://console.cloud.google.com"] {
            ...
            assert!(err.contains("GIPHY"), "{key:?} gave: {err}");
