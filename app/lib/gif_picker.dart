...
//    is what turning it on means: searches reach GIPHY (over
...
class _GifSheetState extends State<_GifSheet> {
  ...
  /// No key, no request. Asking without one comes back as a bare 401, which
  ...
  Future<void> _search(String q) async {
    ...
    try {
      ...
      setState(() {
        // A key that GIPHY rejects lands here; send the user back to the panel
        // that explains how to get a working one instead of leaving them with
        // the raw refusal.
        _needsKey = message.contains('GIPHY') || message.contains('API key');
        _error = message;
        _loading = false;
      });
    }
